...
#    ys = np.concatenate((tigers_y, cats_y), axis=0)
    ...
    labels_vecs = lb.transform(ys)
    vgg = Vgg16(vgg16_npy_path='./for_transfer_learning/vgg16.npy')
    print('Net built')
    epochs=100
    for i in range(epochs):
        for x,y in get_batches(xs,labels_vecs):
            #b_idx = np.random.randint(0, len(xs), 100)
            train_loss = vgg.train(x,y)
           # print(i, 'train loss: ', train_loss)
            print( i,'train loss: ', train_loss)
    vgg.save('./for_transfer_learning/flower_learn')      # save learned fc layers
